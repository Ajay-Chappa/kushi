...
import { dashboardStats,mockServices, mockCustomers } from '../data/mockData';
...
useEffect(() => {
  fetch("http://localhost:8081/api/bookings/recent-bookings") // ✅ correct backend URL
    .then((res) => res.json())
    .then((data) => {
  ...
  const quickStats = [
    { label: 'Today\'s Bookings', value: '12', icon: Calendar, color: 'text-blue-600' },
    { label: 'Pending Approvals', value: '5', icon: Clock, color: 'text-yellow-600' },
    { label: 'Active Staff', value: '24', icon: Users, color: 'text-green-600' },
    { label: 'Completion Rate', value: '98%', icon: Target, color: 'text-purple-600' }
  ];
          ...
          <CardContent className="p-3 md:p-4">
            <div className="space-y-3">
              {topServices.map((service, index) => (
                <div key={service.id} className="group flex items-center space-x-3 p-3 rounded-xl bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 hover:shadow-md transition-all duration-300 border border-gray-200 dark:border-gray-600 cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-gray-900 dark:text-white group-hover:text-primary-600 transition-colors truncate text-sm">{service.title}</h4>
          ...
          <CardContent className="p-3 md:p-4">
            <div className="space-y-3">
              {topCustomers.map((customer, index) => (
                <div key={customer.id} className="group flex items-center space-x-3 p-3 rounded-xl bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 hover:shadow-md transition-all duration-300 border border-gray-200 dark:border-gray-600 cursor-pointer">
                  <div className="relative">
                    <div className="w-12 h-12 bg-gradient-to-br from-primary-500 to-coral-500 rounded-xl flex items-center justify-center shadow-md group-hover:scale-105 transition-transform duration-300">
                      <span className="text-white font-bold text-sm">
                        {customer.name.split(' ').map(n => n[0]).join('')}
                      </span>
                    </div>
                    <div className="absolute -top-1 -right-1 bg-gradient-to-r from-coral-600 to-primary-600 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center shadow-lg">
                      {index + 1}
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-gray-900 dark:text-white group-hover:text-primary-600 transition-colors truncate text-sm">{customer.name}</h4>
                    <div className="flex items-center space-x-3 mt-1">
