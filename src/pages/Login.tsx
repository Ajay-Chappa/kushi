...
export function Login() {
  ...
  if (isLogin) {
    try {
      const res = await fetch('http://localhost:8081/api/login/login', {
  method: 'POST',
...
if (res.ok && msg.includes("Login successful")) {
  ...
  setError(msg || 'Invalid credentials.');
    ...
    if (
      ...
      try {
        const res = await fetch('http://localhost:8081/api/login/register', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: formData.name,
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm">
          <CardContent className="p-8">
            ...
            {/* Demo Credentials */}
            {isLogin && (
              <div className="mt-6 p-4 bg-gradient-to-r from-primary-50 to-coral-50 rounded-lg border border-primary-200">
                <p className="text-sm font-medium text-gray-700 mb-2">Demo Credentials:</p>
                <div className="flex flex-col space-y-2">
                  <button
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, email: 'admin@kushiservices.com', password: 'admin123' }))}
                    className="text-left p-2 bg-white rounded border hover:bg-gray-50 transition-colors"
                  >
                    <p className="text-xs font-medium text-gray-700">Click to use demo credentials:</p>
                    <p className="text-xs text-gray-600">Email: admin@kushiservices.com</p>
